...
| :--- | :--- | :--- |
| **Critical** | All key/value lines in `[Difficulty]`; `AudioFilename` in `[General]`; positive `[TimingPoints]` (uninherited / BPM), compared on their timing-significant fields only — `time`, `beatLength`, `meter`, and the uninherited flag | **Reject the upload** with a frontend modal. Nothing is written. The uploader either cancels or confirms — see "Acknowledged critical mismatch" below. **Only the mapset `owner` can permanently change critical settings.** |
...
A positive point that differs from the base **only** in its hitsounding group (`sampleSet`, `sampleIndex`), `volume`, or `effects` (kiai / omit-barline) is **not** critical — those are per-difficulty cosmetic settings, not shared timing. The comparison ignores them. This is safe for any point the uploaded section actually carries: the merge engine lets a section's positive point win over the base's at the same offset, so a mapper's hitsounding/volume/effects change reaches the final map without promoting the base. A full-difficulty upload carries every positive, so this always holds there; a single-section upload only carries the positives inside its `[startMs, endMs)` range, so a cosmetic change to a positive *outside* that range is dropped (the trimmed section doesn't store it and the base isn't promoted) — acceptable, since cross-section timing cosmetics are the base's concern, not a single section's.
